...
- **AV 口径更新（2026-09-07）：** 下列 M04/M05 行保留 2026-08-08 的实测与当时措辞；其中“需在 AV 环境扫描”不再是硬性完成条件。当前统一采用 `PASS/FAIL/NOT_RUN-SKIPPED_BY_USER` 三态，`FAIL` 阻塞、明确跳过不阻塞且不等于通过，详细字段见 `RELEASE_CHECKLIST.md` §4.1 与 `08` §36。
- fixture：固定输入基准的规范由各自 M 定义（M02 长文本/崩溃切点、M03 六格式/旧库/容器、M04 绿色版）；M01 冻结其命名与证据位置约定，不预置未验证数据。
  ...
  - **M03（P1-T05-M03，2026-08-08 实测）**：六格式 golden fixture = 模块级固定字节（TXT/JSON/SRT/VTT/ASS/SSA 各一，`tests/test_p1_t05_m03.py` 内常量，`test_golden_fixtures_are_fixed_and_recorded` 断言精确字节大小 + sha256 防漂移，段数：txt 3/json 3/srt 2/vtt 2/ass 1/ssa 1）；旧库 fixture = `_apply_old_migrations` 仅应用 001-007 并写入真实 project/document/segment 数据；容器 fixture = 六格式 golden 全导入的开放目录 + 只存 `env:` 引用的 connection + profile。旅程级矩阵记录见 `tests/test_p1_t05_m03.py`（15 项：六格式 no-op 逐字节、六格式翻译仅改目标 span + round-trip、单格式缺载体/不可写失败不污染、旧库迁移备份先于 008 且可恢复/恢复后重开/备份失败阻断、六格式容器跨机保留 fidelity carrier、篡改/越界/超限额落地前拒绝、源码/配置无硬编码凭据、archive/目标只含引用、失败隔离与状态可解释）。
  - **M04（P1-T05-M04，2026-08-08 实测）**：绿色版候选 = `scripts/build_release.py` 从锁定环境（Python 3.12.10 / PyInstaller 6.21.0 / PySide6 6.11.1，均等于 `requirements.lock`）构建的 onedir `dist/transrealm/` + zip `dist/transrealm-0.1.0-win-x64.zip` + `dist/build_manifest.json`（记录 tool/version/git commit/hash/size/迁移数/Qt 插件）。候选身份 = git commit `110dea9` + 锁文件共同可定位（`09` §6）；artifact：zip 51,676,076 字节/sha256 `c59b3ebbe…`（顶层仅 `transrealm/`，`base_dir` 限定不含 `.work/` 构建残留与自引用 zip）、onedir 128,322,729 字节、exe sha256 `cffc576f…`、12 迁移、Qt 插件 8 类（generic/iconengines/imageformats/networkinformation/platforminputcontexts/platforms/styles/tls）+ `platforms/qwindows.dll`。启动证据见 `tests/test_p1_t05_m04.py`（4 项：构建工具不变量、manifest-artifact 一致 + 冻结 12 迁移 + Qt 平台插件 + zip hash/size、绿色 artifact 纯净（顶层仅 onedir、无数据后缀/`.env`/`.work`/嵌套 zip/陈旧 manifest，放行 PyInstaller 内部 `_internal/base_library.zip`）、冻结 unzip→launch→数据目录（USERPROFILE 覆盖）→12 迁移→WM_CLOSE 优雅退出（PID 窗口定位 + `returncode==0`）→数据完好→删程序目录不删外置数据 smoke）；独立手动核验冻结应用 0.61s 内出窗口 + 12 迁移 + 12 业务表、优雅退出数据完好。**未覆盖项：** 杀毒扫描本机不可执行（Windows Defender 实时保护禁用 `AntivirusEnabled=False` 且非管理员，`MpCmdRun` hr=0x80004005），需用户在 AV 环境扫描候选后归档；冻结应用内 create/import/translate/export 的 GUI 自动化驱动未落地（以冻结启动/迁移/关闭 smoke + P1-T03/P0-T08 源码级真实窗口完整旅程为证据）。
  - **M05（P1-T05-M05，2026-08-08 实测）**：文档交付型 Milestone，无产品代码改动。新增 `docs/`（`user-guide.md` 安装/使用/模式/Profile/Glossary/导出/清理、`data-safety.md` 迁移/备份/恢复/回滚/凭据、`known-issues.md` 候选已知问题与未覆盖项、`third-party-licenses.md` 依赖许可）+ `README.md` 用户文档小节（链接四份文档）+ `tests/test_p1_t05_m05.py` **4 项文档一致性守卫**（四份文档存在且非空；README 含四份链接；`user-guide.md` 数据目录 `%USERPROFILE%\.transrealm` 与 `main_window.py` `Path.home() / ".transrealm"` 一致；`known-issues.md` zip sha256/版本与 `dist/build_manifest.json` 一致——缺候选 skip 并提示构建命令，同 M04 模式）。证据位置：`docs/`、`README.md`、`07` §20、`08`、`DEVELOPMENT_STATE.md`；M05 未覆盖项承接 M04（杀毒扫描可执行或明确跳过并按三态留痕、冻结 GUI 自动化最终人工 smoke 需用户授权发布时执行）。
